...
import { Plus, Trash2, Briefcase, Edit2, Check } from 'lucide-react'
...
export default function IncomeStreamForm() {
  ...
  function startEdit(s) {
    ...
    setForm({
      ...
      lump_sum_amount: s.lump_sum_amount?.toString() || '',
    })
  }
  ...
  async function save() {
    ...
    let u
    if (editId) {
      u = incomeStreams.map(s => s.id === editId ? { ...updatedData, id: editId } : s)
    } else {
      u = [...incomeStreams, { ...updatedData, id: uuidv4() }]
    }
  ...
  return (
    <div className="card p-5 sm:p-6">
      <div className="flex items-center justify-between mb-5">
        ...
        {!showAdd && (
          <button onClick={() => setShowAdd(true)} className="flex items-center gap-2 px-4 py-2.5 rounded-full bg-g-blue text-white font-body text-sm font-medium hover:bg-[#3367d6] transition-all shadow-sm">
            <Plus size={16} /> Add
          </button>
        )}
      </div>
        ...
        {incomeStreams.map(s => (
          <div key={s.id} className={`flex items-center gap-3 px-4 py-3.5 rounded-2xl border transition-all ${s.is_active ? 'bg-g-surface border-g-border hover:border-g-blue/30' : 'bg-g-bg border-g-border opacity-50'} ${editId === s.id ? 'ring-2 ring-g-blue border-g-blue' : ''}`}>
            ...
            <div className="flex items-center gap-1">
              <button onClick={() => startEdit(s)} className="text-g-text-tertiary hover:text-g-blue transition-colors p-1.5 rounded-xl hover:bg-g-blue-pastel flex-shrink-0"><Edit2 size={16} /></button>
      ...
      {showAdd && (
        <div className="border border-g-border rounded-2xl p-5 space-y-4 bg-g-bg/60 slide-in">
          <div className="flex items-center justify-between mb-1">
            <h4 className="font-display font-bold text-g-text text-sm">{editId ? 'Edit Income Source' : 'Add Income Source'}</h4>
          </div>
